...
const minuteData = flattenedReadings
  ...
  .reduce((acc, reading) => {
    ...
    // Daily Data (averaged over multiple days) - Modified to scale AC current by 100
      ...
      .reduce((acc, reading) => {
        ...
        if (existing) {
          ...
          acc.push({
            ...
            acVoltage: reading.ACvoltage,
            acCurrent: reading.ACcurrent,
            acCurrentScaled: reading.ACcurrent * 100, // Scale by 100 instead of storing raw value
            count: 1
          });
        }
  ...
  // Custom formatter for tooltip - Updated for daily view
  const formatDailyTooltipValue = (value, name, props) => {
    if (name === 'AC Current (A × 100)') {
      return [`${(props.payload.acCurrent).toFixed(3)} A`, 'AC Current (Actual)'];
    }
    return [`${value.toFixed(2)} ${name.includes('Voltage') ? 'V' : 'A'}`, name];
  };
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          ...
          <div className="flex items-center space-x-2 w-full sm:w-auto">
            <input
              ...
              className="flex-1 sm:flex-none px-3 py-1 bg-slate-700 border border-slate-600 rounded text-sm text-slate-300"
            />
            <Button
              variant="outline"
              size="sm"
      ...
      <div className="p-3 sm:p-6">
        <Tabs defaultValue="hourly" className="space-y-4 sm:space-y-6">
          ...
          <TabsList className="bg-slate-800 border-slate-700 grid grid-cols-2 sm:grid-cols-5 w-full">
            <TabsTrigger value="hourly" className="data-[state=active]:bg-slate-700 text-xs sm:text-sm">
              Hourly
            </TabsTrigger>
            <TabsTrigger value="daily" className="data-[state=active]:bg-slate-700 text-xs sm:text-sm">
              Daily
            </TabsTrigger>
            <TabsTrigger value="weekly" className="data-[state=active]:bg-slate-700 text-xs sm:text-sm">
              Weekly
            </TabsTrigger>
            <TabsTrigger value="circuits" className="data-[state=active]:bg-slate-700 text-xs sm:text-sm">
              Circuits
            </TabsTrigger>
            <TabsTrigger value="performance" className="data-[state=active]:bg-slate-700 text-xs sm:text-sm">
              Performance
            </TabsTrigger>
          </TabsList>
...
export default Charts;siveContainer>
                </CardContent>
              </Card>
                ...
                <CardContent>
                  <ResponsiveContainer width="100%" height={250} className="sm:h-[300px]">
                    <BarChart data={dailyData}>
                      ...
                      <Tooltip 
                        contentStyle={{ backgroundColor: '#1e293b', border: '1px solid #475569', fontSize: '12px' }}
                        formatter={formatDailyTooltipValue}
                      />
                      <Legend wrapperStyle={{ fontSize: '12px' }} />
                      <Bar
                        dataKey="acVoltage"
